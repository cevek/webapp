...
function isLineTerminator(cp: number) {
    return (cp === 0x0A) || (cp === 0x0D) || (cp === 0x2028) || (cp === 0x2029);
}

function isIdentifierStart(cp: number) {
...
function isIdentifierPart(cp: number) {
    return (cp === 0x24) || (cp === 0x5F) ||  // $ (dollar) and _ (underscore)
        (cp >= 0x41 && cp <= 0x5A) ||         // A..Z
        (cp >= 0x61 && cp <= 0x7A) ||         // a..z
        (cp >= 0x30 && cp <= 0x39) ||         // 0..9
...
const nextNewExpressionStartSymbols: boolean[] = [];

'~%^&*/-+([;:?,<>=|'.split('').forEach(ch => {
    nextNewExpressionStartSymbols[ch.charCodeAt(0)] = true;
});
function isNextStartExpression(i: number) {
...
export function parseJS(code: string, patternMatcher: (code: string, start: number, size: number, startSymbolCode: number)=>boolean) {
    let i = -1;
